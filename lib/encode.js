...
// Cache regex's
var rImages = /([\s\S]*?)((?:(?:src\s*=\s*|url)[\'|\"|\(])([^>\s)]+)[\'|\"|\)])(?!\s*[;,>]?\s*\/\*\s*base64:skip\s*\*\/)|([\s\S]+)/img;
      ...
      if(group[4] == null) {
        ...
        if (opts.extensions) { //test for extensions if it provided
          ...
          if (typeof opts.extensions === 'function') {
            ...
            test = opts.extensions.some(function (ext) {
              return (ext instanceof RegExp) ? ext.test(rawUrl.replace(rQuotes, "")) : (ext === imgExt);
            });
          }
        }
        ...
        if(cache[img]) {
          ...
          exports.image(loc, opts, function (err, resp, cacheable) {
            if (err == null) {
              // var url = "url(" + resp + ")";
              var url = group[2].replace(group[3],resp);
              result += url;
